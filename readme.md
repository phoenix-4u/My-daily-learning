...
- GANs are sampling-based models, i.e., given random noise z, they can predict p(x)
- GANs play a 2 player game where generators generate an image from noise z, and the discriminator detects whether the generator generated a real or fake image.
- The training objective of the discriminator is to maximize the loss between real and fake images, whereas the generator's objective is to minimize the loss between its output and the discriminator's loss
- <img width="1372" height="685" alt="image" src="https://github.com/user-attachments/assets/2f50b86d-b1a5-4ca4-b5d3-c80f7bd7173b" />
- This works with the help of Jensen-Shannon Divergence, which tries to minimize the loss between the distribution of images generated by the generator and the distribution of real images
- They are very difficult to optimize, as for every step of G and run an optimizer pass for D. So people pray for convergence.
- GANs have been used for image resolution enhancement where only the discriminator is used to classify a GAN loss.
...
- This works with KL divergence. So the idea is to draw a sample x from a Gaussian distribution with a mean close to zero and a standard deviation close to 1 and then predict z
...
- Generative models try to learn the probability distribution over certain data samples. It does not come with any training set.
...
- A float32 precision contains a sign with 1 bit, an exponent with 8 bits, and a fractional part of 23 bits
- A float32 is enough to cover an LLM's parameters, weights, gradients, and activations for a model like Llama 3.1
- A float16 precision contains a sign with 1 bit, an exponent with 5 bits, and a fractional part of 10 bits. This restricts the max and min range
- A bfloat16 precision contains a sign with 1 bit, an exponent with 8 bits, and a fractional part of 7 bits. This has a big range, but the precision suffers
- Matrix multiplication can be implemented in block form; switching from float32 to bfloat16 ensures that speed is up by a factor of 2, as twice as many bfloats can fit into the block compared to float32
